...
struct bitstr_base {
    static bool test(const char *buf, unsigned bit) noexcept {
        return (buf[bit_byte(bit)] & bit_mask(bit)) != 0;
    }
    static bool test_clear(char *buf, unsigned bit) noexcept {
        unsigned mask = bit_mask(bit);
        char *p = buf + bit_byte(bit);
        if (*p & mask) {
            *p &= ~mask;
            return true;
        }
...
public:
    ...
    bool test_clear(unsigned bit) noexcept {
        return bitstr_base::test_clear(_buf, bit);
    }

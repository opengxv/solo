...
struct BindType<const char*> {
    static constexpr const size_t strsize = 256;
    char _value[strsize];
    unsigned long _length;
    void bind(MYSQL_BIND *item) noexcept {
...
template <>
struct BindType<const std::string&> {
    static constexpr const size_t strsize = 256;
    char _value[strsize];
    unsigned long _length;
    void bind(MYSQL_BIND *item) noexcept {
...
protected:
    static constexpr const size_t arg_count = sizeof...(_Args);

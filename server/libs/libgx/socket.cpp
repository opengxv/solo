...
bool Connector::connect_handler(Socket &socket, unsigned type) {
    ...
    if (type & (Reactor::poll_out | Reactor::poll_err)) {
        ...
        if (getsockopt(_socket->fd(), SOL_SOCKET, SO_ERROR, (char*)&n, &len) >= 0) {
#else
...
bool Listener::listen() noexcept {
    ...
    int n = 1;
#ifdef GX_PLATFORM_WIN32 
	if (::setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&n, sizeof(int))) {
#else

...
#define SSCC_CONSTRUCT(_T)          sscc_pool->construct<_T>(sscc_pool)
...
#define SSCC_WRITE_SIZE(x)                           \
do {                                                 \
    unsigned __size = (x);                           \
    if ((__size & (~0x7f)) == 0) {                   \
        __size <<= 1;                                \
        sscc_stream.write((uint8_t)__size);          \
    }                                                \
    else if ((__size & (~0x7fff)) == 0) {            \
        __size <<= 1;                                \
        __size |= 1;                                 \
        sscc_stream.write((uint16_t)__size);         \
    }                                                \
    else {                                           \
        return false;                                \
    }                                                \
...
#define SSCC_READ_SIZE(x)                            \
    do {                                             \
        size_t __size;                               \
        SSCC_READ_UINT8(__size);                     \
        if (__size & 1) {                            \
            unsigned __ext;                          \
            SSCC_READ_UINT8(__ext);                  \
            __size |= (__ext << 8);                  \
        }                                            \
        x = (__size >> 1);                           \
    } while (0)
